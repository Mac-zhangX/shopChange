<template>
  <div style="padding:1rem">
    ...
    <div>
      <el-button v-if="checkedAll" size="small" class="el-icon-edit" type="danger" @click="sh()">批量审核</el-button>
    </div>
    <div style="margin-top:1rem">
      <div class="mine">
        ...
        >
          ...
          <el-table-column prop="AuditName" label="认证状态" />
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      this.$axios.post(UrlHelper.realName.query, formData).then(val => {
        if (val.data.Code === 200) {
          ...
          if (val1.Audit === 0) {
           val1.AuditName = '未认证'
          } else if (val1.Audit === 1) {
            val1.AuditName = '待审核'
          } else if (val1.Audit === 2) {
            val1.AuditName = '审核通过'
          }
          });
    ...
    sh(){
     ...
     // console.log(this.multipleSelection)
      ...
      this.$axios.post(UrlHelper.realName.realAudit,format).then(val=>{
        if(val.data.Code===200){
              this.$message({
                    type: "success",
                    message: val.data.Msg,
    ...
    check(data){
        ...
        this.$axios.post(UrlHelper.realName.realAudit,formData).then(val=>{
        if(val.data.Code===200){
              this.$message({
                    type: "success",
                    message: val.data.Msg,
