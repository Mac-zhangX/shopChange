<template>
  <div style="padding:1rem">
    ...
    >
      ...
      <!-- <el-table-column label="openid" width="220">
      ...
      <el-table-column prop="province" label="归属地（省）" />
      <el-table-column prop="city" label="归属地（市）" />
      <el-table-column prop="commission" label="佣金" />
      <el-table-column prop="discount" label="折扣" />
      <el-table-column label="头像">
        <template slot-scope="scope"><img style="width:80px" :onerror="defaultImg" :src="scope.row.headimgurl" @click="showImg(scope.row.headimgurl)"></template>
      </el-table-column>
    ...
    <div>
      <el-dialog :close-on-click-modal="false" width="30%" title="头像" :visible.sync="imgdialogFormVisible" @close="close()">
        <img style="width: 100%" :src="nowUrl">
      </el-dialog>
    ...
    <div>
      <el-dialog :close-on-click-modal="false" width="40%" :title="diaTitle" :visible.sync="dialogFormVisible" @close="close()">
        <el-form ref="userEntity" :rules="rules" :model="userEntity">
          ...
          <el-form-item prop="discount" label="折扣" :label-width="formLabelWidth">
            <el-input v-model="userEntity.discount" type="number" min="0" style auto-complete="off" />
          </el-form-item>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      nowUrl: '', // 当前的图片链接"../../assets/image/bacB.jpg
  ...
  methods: {
    ...
    getList() {
      // console.log('----')
      // 查询所有的订单信息
      this.$axios.post(UrlHelper.weixinUser.all, {
      ...
      this.$refs['userEntity'].validate((valid) => { // 进行验证规则的验证
        ...
        if (valid) {
          if (this.diaTitle === '添加用户') {
            ...
            console.log(reg.test(this.userEntity.discount))
            if (!reg.test(this.userEntity.discount)) {
              this.$message({
                type: 'error',
                message: '请输入正确的数字',
                duration: 1000
            ...
            this.$axios.post(UrlHelper.weixinUser.update, this.userEntity).then(val => {
              if (val.data.code === 200) { // 如果请求成功
                this.$message({
                  type: 'success',
                  message: val.data.msg,
