<template>
  <div style="padding:1rem">
    <div class="filter">
      <div>
        手机号&nbsp;&nbsp;
        <el-input v-model="input" size="small" class="input1" placeholder="请输入内容" />
      </div>
      <div>
        开始时间&nbsp;&nbsp;
        <el-date-picker
    ...
    <div>
      <el-button v-if="checkedAll" size="small" class="el-icon-edit" type="danger" @click="chenggong()">批量审核成功</el-button>
      <el-button v-if="checkedAll" size="small" class="el-icon-edit" type="warning" @click="shibai()">批量审核失败</el-button>
    </div>
    <div style="margin-top:1rem">
      <div class="mine">
        ...
        >
          ...
          <el-table-column prop="Phone" label="银行预留手机号" />
      ...
      >
        ...
        >
          ...
          <el-form-item label="是否提交" prop="sflag">
            <!-- 1是未提交2是提交 -->
            <el-radio v-model="ruleForm.sflag" :label="1">等待处理</el-radio>
            <el-radio v-model="ruleForm.sflag" :label="2">处理成功</el-radio>
            <el-radio v-model="ruleForm.sflag" :label="3">处理失败</el-radio>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      this.$axios.post(UrlHelper.drawMoney.getList, formData).then(val => {
    ...
    chenggong(){
      ...
      this.$axios.post(UrlHelper.drawMoney.cashAudit,format).then(val=>{
        if(val.data.Code===200){
              this.$message({
                    type: "success",
                    message: val.data.Msg,
    ...
    //  this.$axios.post(UrlHelper.orderList. orderWrite)
         this.$axios.post(UrlHelper.drawMoney.cashError,format).then(val=>{
        if(val.data.Code===200){
              this.$message({
                    type: "success",
                    message: val.data.Msg,
